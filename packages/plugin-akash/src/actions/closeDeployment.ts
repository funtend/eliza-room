...
async function closeAllDeployments(
    ...
    try {
        ...
        const deployments = await fetchDeployments(runtime, undefined, 0, 100);
        const activeDeployments = deployments.results.filter(
        ...
        try {
            ...
            if (!params.dseq && !params.closeAll) {
                if (config.AKASH_CLOSE_DEP === "closeAll") {
                    ...
                    config.AKASH_CLOSE_DEP === "dseq" &&
                    config.AKASH_CLOSE_DSEQ
                ) {
    ...
    handler: async (
        ...
        try {
            ...
            if (params.closeAll) {
                ...
                if (callback) {
                    elizaLogger.info(
                        "=== Preparing callback response for bulk closure ===",
                        {
                            hasCallback: true,
                            actionId,
                ...
                const success = await closeSingleDeployment(
                    runtime,
                    params.dseq
                );
                ...
                if (callback) {
                    elizaLogger.info(
                        "=== Preparing callback response for single closure ===",
                        {
                            hasCallback: true,
                            actionId,
                            dseq: params.dseq,
