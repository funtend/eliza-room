...
function loadCertificate(): CertificatePem {
    elizaLogger.debug("Loading certificate from file", {
        path: CERTIFICATE_PATH,
    });
    try {
    ...
    for (const endpoint of rpcEndpoints) {
        try {
            ...
            elizaLogger.warn("Failed to connect to RPC endpoint", {
                endpoint,
                error: error instanceof Error ? error.message : String(error),
                remainingEndpoints: rpcEndpoints.slice(
                    rpcEndpoints.indexOf(endpoint) + 1
                ).length,
            });
        }
    }
...
export const createCertificateAction: Action = {
    ...
    handler: async (
        ...
        elizaLogger.info("=== Starting Certificate Creation/Loading ===", {
            actionId,
        });
        ...
        try {
            ...
            try {
                ...
                // Initialize wallet
                elizaLogger.info(
                    "Initializing wallet for certificate creation"
                );
