...
const loadSDLFromFile = (filePath: string): string => {
    ...
    try {
        ...
        if (
            !filePath.includes("plugin-akash") &&
            filePath !== DEFAULT_SDL_PATH
        ) {
            const adjustedPath = path.join(
                path.dirname(DEFAULT_SDL_PATH),
...
async function queryLeaseStatus(
    ...
    if (id === undefined) {
        ...
        throw new AkashError(
            "Lease ID is undefined",
            AkashErrorCode.INVALID_LEASE
        );
    }
        ...
        try {
            ...
            try {
                ...
                if (response.status !== 200) {
                    elizaLogger.warn(
                        "Non-OK response from lease status query",
                        {
                    ...
                    throw new Error(
                        `Could not query lease status: ${response.status}`
                    );
                }
    ...
    try {
        ...
        try {
            ...
            if (response.status !== 200) {
                ...
                throw new Error(
                    `Failed to send manifest: ${response.status} ${response.statusText}`
                );
            }
            ...
            try {
                ...
                if (status === undefined) {
                    ...
                    if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
                        elizaLogger.warn(
                            "Too many consecutive undefined status responses",
                            {
                                dseq,
                                consecutiveErrors,
                            }
                ...
                for (const [name, service] of Object.entries<{
                    ...
                    if (service.uris) {
                        ...
                        const serviceUrl = rawUrl.startsWith("http")
                            ? rawUrl
                            : `http://${rawUrl}`;
...
async function loadOrCreateCertificate(
    ...
    try {
        ...
        if (fs.existsSync(CERTIFICATE_PATH)) {
            elizaLogger.info("Found existing certificate file", {
                path: CERTIFICATE_PATH,
            });
        ...
        elizaLogger.info("No existing certificate found, creating new one", {
            address,
        });
        ...
        const result = await cert
            .broadcastCertificate(certificate, address, client as any)
            .catch((error) => {
        ...
        elizaLogger.info("Certificate saved to file", {
            path: CERTIFICATE_PATH,
        });
...
export const createDeploymentAction: Action = {
    ...
    validate: async (
        ...
        if (!isPluginLoaded(runtime, "akash")) {
            elizaLogger.error(
                "Akash plugin not properly loaded during validation"
            );
            return false;
        }
        ...
        try {
            ...
            try {
                ...
                if (yamlSeparatorIndex === -1) {
                    throw new Error(
                        "No YAML document separator (---) found in SDL"
                    );
                }
                ...
                elizaLogger.error("SDL format validation failed", {
                    error: sdlError,
                });
                throw new AkashError(
        ...
        try {
            ...
            const deployment = {
                ...
                deposit: {
                    denom: "uakt",
                    amount:
                        params.deposit?.replace("uakt", "") ||
                        config.AKASH_DEPOSIT.replace("uakt", ""),
                },
                version: await sdl.manifestVersion(),
                depositor: address,
            };
            ...
            const result = await withRetry(async () => {
                elizaLogger.debug(
                    "Attempting to sign and broadcast transaction",
                    {
                        attempt: "current",
                ...
                const txResult = await client.signAndBroadcast(address, [msg], {
                    amount: [
                        {
                            denom: "uakt",
                            amount: config.AKASH_DEPOSIT.replace("uakt", ""),
                        },
                    ],
                    gas: "800000",
                });
            ...
            const serviceUrl = await sendManifest(
                sdl,
                lease,
                certificate,
                config.RPC_ENDPOINT
            ...
            if (callback) {
                elizaLogger.info(
                    "=== Preparing callback response for deployment creation ===",
                    {
                        hasCallback: true,
                        actionId,
            ...
            if (callback) {
                elizaLogger.info("=== Preparing error callback response ===", {
                    ...
                    hasCallback: true,
                    errorType:
                        error instanceof AkashError ? "AkashError" : "Error",
                });
                ...
                elizaLogger.info(
                    "=== Error callback executed successfully ===",
                    {
                        actionId,
                        timestamp: new Date().toISOString(),
                    }
