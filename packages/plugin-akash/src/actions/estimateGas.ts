...
export const estimateGas: Action = {
    ...
    validate: async (
        ...
        try {
            ...
            if (params.text && !params.dseq) {
                const dseqMatch =
                    params.text.match(/dseq\s*(?::|=|\s)\s*(\d+)/i) ||
                    params.text.match(
                        /deployment\s+(?:number|sequence|#)?\s*(\d+)/i
                    ) ||
            ...
            if (!params.operation) {
                if (config.AKASH_GAS_OPERATION) {
                    params.operation = config.AKASH_GAS_OPERATION as
                        | "close"
                        | "create"
                        | "update";
                    elizaLogger.info("Using operation from environment", {
            ...
            if (params.operation === "close") {
                if (!params.dseq) {
                    if (config.AKASH_GAS_DSEQ) {
                        ...
                        elizaLogger.info("Using DSEQ from environment", {
                            dseq: params.dseq,
                        });
                    } else {
                        throw new AkashError(
    ...
    handler: async (
        ...
        try {
            ...
            if (callback) {
                elizaLogger.info(
                    "=== Preparing callback response for gas estimation ===",
                    {
                        hasCallback: true,
                        actionId,
                        operation: params.operation,
                ...
                const operationText =
                    params.operation === "close"
                        ? `closing deployment ${params.dseq}`
                        : params.operation;
                const estimateData = {
                    gasEstimate,
                    operation: params.operation,
                ...
                elizaLogger.warn(
                    "=== No callback provided for gas estimation ===",
                    {
                        actionId,
                        operation: params.operation,
                        dseq: params.dseq,
            ...
            if (callback) {
                ...
                elizaLogger.warn(
                    "=== No callback provided for error handling ===",
                    {
                        actionId,
                        errorMessage:
