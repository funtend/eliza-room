...
async function fetchWithRetry(
    ...
    for (let i = 0; i < retries; i++) {
        try {
            ...
            elizaLogger.warn(
                `API request failed (attempt ${i + 1}/${retries})`,
                {
            ...
            elizaLogger.warn(
                `API request error (attempt ${i + 1}/${retries})`,
                {
                    error:
...
): Promise<{ wallet: DirectSecp256k1HdWallet | null; address: string }> {
    try {
...
export async function fetchDeployments(
    ...
    try {
        ...
        if (!fs.existsSync(deploymentDir)) {
            elizaLogger.info("Creating deployments directory", {
                deploymentDir,
            });
        ...
        const deploymentsByStatus = data.results.reduce(
            (acc, deployment) => {
        ...
        for (const [status, deployments] of Object.entries(
            deploymentsByStatus
        )) {
            const statusDir = path.join(deploymentDir, status);
            elizaLogger.info("Processing status directory", {
                statusDir,
                status,
            ...
            await Promise.all(
                deployments.map(async (deployment) => {
                    const filePath = path.join(
                        statusDir,
                        `${deployment.dseq}.json`
                    );
                    elizaLogger.debug("Saving deployment file", {
                        filePath,
...
export const getDeploymentApiAction: Action = {
    ...
    validate: async (
        ...
        try {
            ...
            if (
                params.status &&
                !["active", "closed"].includes(params.status)
            ) {
                throw new AkashError(
            ...
            if (
                ...
                (typeof params.skip !== "number" || params.skip < 0)
            ) {
                throw new AkashError(
    ...
    handler: async (
        ...
        try {
            ...
            if (callback) {
                elizaLogger.info(
                    "=== Preparing callback response for deployments ===",
                    {
                        hasCallback: true,
                        actionId,
