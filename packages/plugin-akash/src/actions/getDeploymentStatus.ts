...
export const getDeploymentStatusAction: Action = {
    ...
    validate: async (
        ...
        elizaLogger.debug("Validating get deployment status request", {
            message,
        });
        try {
            const params =
                message.content as Partial<GetDeploymentStatusContent>;
            ...
            if (!params.dseq) {
                if (
                    config.AKASH_DEP_STATUS === "dseq" &&
                    config.AKASH_DEP_DSEQ
                ) {
                    params.dseq = config.AKASH_DEP_DSEQ;
                } else if (config.AKASH_DEP_STATUS === "param_passed") {
                    elizaLogger.info(
                        "DSEQ parameter is required when AKASH_DEP_STATUS is set to param_passed",
                        {
                    ...
                    elizaLogger.info(
                        "No DSEQ provided and no valid environment configuration found",
                        {
    ...
    handler: async (
        ...
        try {
            ...
            const wallet = await DirectSecp256k1HdWallet.fromMnemonic(
                config.AKASH_MNEMONIC,
                { prefix: "akash" }
            );
            const [account] = await wallet.getAccounts();

            // Initialize query client
            const queryClient = new DeploymentQueryClient(
                await getRpc(config.RPC_ENDPOINT)
            );
            ...
            try {
                ...
                const deployment =
                    response.deployment as unknown as DeploymentResponse;
                const status = {
                ...
                if (callback) {
                    ...
                    const stateString =
                        DeploymentState[
                            status.state as keyof typeof DeploymentState
                        ] || "UNKNOWN";
                    ...
                    elizaLogger.info(
                        "=== Preparing callback response for deployment status ===",
                        {
                            hasCallback: true,
                            actionId,
                            dseq: params.dseq,
                    ...
                    elizaLogger.info(
                        "=== Executing callback with response ===",
                        {
                            actionId,
                ...
                if (dseqSource === "environment") {
                    elizaLogger.warn(
                        "Failed to query deployment from environment configuration",
                        {
                            dseq: params.dseq,
                            error:
                        ...
                        callback({
                            ...
                            content: {
                                ...
                                error: {
                                    ...
                                    message:
                                        "Invalid AKASH_DEP_DSEQ configuration",
                    ...
                    if (callback) {
                        elizaLogger.info(
                            "=== Preparing error callback response ===",
                            {
                                actionId,
                                hasCallback: true,
                                errorType:
                                    queryError instanceof AkashError
                                        ? "AkashError"
                                        : "Error",
                            }
                        );
