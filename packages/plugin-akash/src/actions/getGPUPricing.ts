...
export const getGPUPricingAction: Action = {
    ...
    description:
        "Get GPU pricing comparison between Akash and major cloud providers",
    examples: [
        [
            {
                user: "user",
    ...
    validate: async (
        ...
        try {
            ...
            if (
                ...
                (isNaN(params.cpu) || params.cpu <= 0)
            ) {
                throw new GPUPricingError(
                    "CPU units must be a positive number",
                    "INVALID_CPU"
                );
            }
            ...
            if (
                ...
                (isNaN(params.memory) || params.memory <= 0)
            ) {
                throw new GPUPricingError(
            ...
            if (
                ...
                (isNaN(params.storage) || params.storage <= 0)
            ) {
                throw new GPUPricingError(
            ...
            elizaLogger.error("GPU pricing validation failed", {
                error:
                    error instanceof GPUPricingError
                        ? {
                              code: error.code,
    ...
    handler: async (
        ...
        try {
            ...
            const savings = {
                ...
                vs_azure: (
                    ((data.azure - data.akash) / data.azure) *
                    100
                ).toFixed(2),
            };
