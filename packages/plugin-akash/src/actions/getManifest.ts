    ...
    try {
        ...
        for (const section of sectionsToCheck) {
            if (!(section in parsed)) {
                throw new Error(
                    `Invalid SDL format: missing required section '${section}'`
                );
            }
        }
...
export const getManifestAction: Action = {
    ...
    description:
        "Load and validate SDL to generate a manifest for Akash deployments",
    examples: [
        [
            {
                user: "user",
        ...
        try {
            ...
            if (params.sdl) {
                const validationLevel =
                    config.AKASH_MANIFEST_VALIDATION_LEVEL || "strict";
        ...
        try {
            ...
            try {
                ...
                if (manifestMode === "validate_only") {
                    elizaLogger.info(
                        "Validation successful (validate_only mode)"
                    );
                    if (callback) {
