...
export const getProviderInfoAction: Action = {
    ...
    validate: async (
        ...
        try {
            ...
            if (params.text && !params.provider) {
                ...
                const providerMatch = params.text.match(
                    /akash1[a-zA-Z0-9]{38}/
                );
                if (providerMatch) {
    ...
    handler: async (
        ...
        try {
            ...
            const queryClient = new ProviderQueryClient(
            ...
            try {
                const response = (await queryClient.Provider(
                    request
                )) as ProviderResponse;
                ...
                const hostUri = response.provider.hostUri.replace(
                    /^https?:\/\//,
                    ""
                );
                elizaLogger.debug("Making provider status request", {
                    url: `https://${hostUri}/status`,
                });

                try {
                    const statusResponse = await fetch(
                        `https://${hostUri}/status`,
                        {
                            headers: {
                ...
                const info = {
                    ...
                    status: response.provider.status
                        ? {
                              nodes: response.provider.status.cluster?.nodes.map(
                                  (node) => ({
                                      name: node.name,
                ...
                if (callback) {
                    elizaLogger.info(
                        "=== Preparing callback response for provider info ===",
                        {
                            hasCallback: true,
                            actionId,
                            provider: params.provider,
            ...
            if (callback) {
                ...
                const errorResponse = {
                    ...
                    details:
                        error instanceof AkashError ? error.details : undefined,
                };
