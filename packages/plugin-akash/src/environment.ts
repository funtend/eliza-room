...
elizaLogger.info("Environment sources", {
    shellVars: Object.keys(process.env).filter((key) =>
        key.startsWith("AKASH_")
    ),
});
...
export const akashEnvSchema = z.object({
    ...
    AKASH_DEPOSIT: z
        ...
        .min(
            1,
            "Deposit is required be careful with the value not too low generally around 500000uakt"
        ),
    ...
    AKASH_MANIFEST_MODE: z
        .string()
        .optional()
        .refine(
            (mode) =>
                !mode || ["auto", "manual", "validate_only"].includes(mode),
            {
                message:
                    "AKASH_MANIFEST_MODE must be one of: auto, manual, validate_only",
            }
        ),
    AKASH_MANIFEST_PATH: z.string().optional(),
    AKASH_MANIFEST_VALIDATION_LEVEL: z
        .string()
        .optional()
        .refine(
            ...
            {
                message:
                    "AKASH_MANIFEST_VALIDATION_LEVEL must be one of: strict, lenient, none",
            }
        ),
});
...
export function getConfig(
    env: string | undefined | null = ENV || process.env.AKASH_ENV
) {
    ENV = env || "mainnet";
    switch (env) {
        case "mainnet":
            return {
                AKASH_NET:
                    "https://raw.githubusercontent.com/ovrclk/net/master/mainnet",
                ...
                AKASH_PRICING_API_URL:
                    process.env.AKASH_PRICING_API_URL ||
                    "https://console-api.akash.network/v1/pricing",
                AKASH_DEFAULT_CPU: process.env.AKASH_DEFAULT_CPU || "1000",
                AKASH_DEFAULT_MEMORY:
                    process.env.AKASH_DEFAULT_MEMORY || "1000000000",
                AKASH_DEFAULT_STORAGE:
                    process.env.AKASH_DEFAULT_STORAGE || "1000000000",
                ...
                AKASH_DEP_STATUS:
                    process.env.AKASH_DEP_STATUS || "param_passed",
                ...
                AKASH_MANIFEST_VALIDATION_LEVEL:
                    process.env.AKASH_MANIFEST_VALIDATION_LEVEL || "strict",
                AKASH_DEPOSIT: process.env.AKASH_DEPOSIT || "500000uakt",
            };
        case "testnet":
            return {
                AKASH_NET:
                    "https://raw.githubusercontent.com/ovrclk/net/master/testnet",
...
export async function validateAkashConfig(
    ...
    try {
        ...
        const akashNet =
            process.env.AKASH_NET ||
            runtime.getSetting("AKASH_NET") ||
            envConfig.AKASH_NET;
        const version = await fetch(`${akashNet}/version.txt`).then((res) =>
            res.text()
        );
        const chainId = await fetch(`${akashNet}/chain-id.txt`).then((res) =>
            res.text()
        );
        const node = await fetch(`${akashNet}/rpc-nodes.txt`).then((res) =>
            res.text().then((text) => text.split("\n")[0])
        );

        // Prioritize shell environment variables over runtime settings
        const mnemonic =
            process.env.AKASH_MNEMONIC || runtime.getSetting("AKASH_MNEMONIC");
        ...
        const mnemonicWords = cleanMnemonic
            .split(" ")
        ...
        const config = {
            ...
            AKASH_GAS_PRICES:
                process.env.AKASH_GAS_PRICES ||
                runtime.getSetting("AKASH_GAS_PRICES") ||
                envConfig.AKASH_GAS_PRICES,
            AKASH_GAS_ADJUSTMENT:
                process.env.AKASH_GAS_ADJUSTMENT ||
                runtime.getSetting("AKASH_GAS_ADJUSTMENT") ||
                envConfig.AKASH_GAS_ADJUSTMENT,
            AKASH_KEYRING_BACKEND:
                process.env.AKASH_KEYRING_BACKEND ||
                runtime.getSetting("AKASH_KEYRING_BACKEND") ||
                envConfig.AKASH_KEYRING_BACKEND,
            AKASH_FROM:
                process.env.AKASH_FROM ||
                runtime.getSetting("AKASH_FROM") ||
                envConfig.AKASH_FROM,
            AKASH_FEES:
                process.env.AKASH_FEES ||
                runtime.getSetting("AKASH_FEES") ||
                envConfig.AKASH_FEES,
            AKASH_PRICING_API_URL:
                process.env.AKASH_PRICING_API_URL ||
                runtime.getSetting("AKASH_PRICING_API_URL") ||
                envConfig.AKASH_PRICING_API_URL,
            AKASH_DEFAULT_CPU:
                process.env.AKASH_DEFAULT_CPU ||
                runtime.getSetting("AKASH_DEFAULT_CPU") ||
                envConfig.AKASH_DEFAULT_CPU,
            AKASH_DEFAULT_MEMORY:
                process.env.AKASH_DEFAULT_MEMORY ||
                runtime.getSetting("AKASH_DEFAULT_MEMORY") ||
                envConfig.AKASH_DEFAULT_MEMORY,
            AKASH_DEFAULT_STORAGE:
                process.env.AKASH_DEFAULT_STORAGE ||
                runtime.getSetting("AKASH_DEFAULT_STORAGE") ||
                envConfig.AKASH_DEFAULT_STORAGE,
            AKASH_SDL:
                process.env.AKASH_SDL ||
                runtime.getSetting("AKASH_SDL") ||
                envConfig.AKASH_SDL,
            AKASH_CLOSE_DEP:
                process.env.AKASH_CLOSE_DEP ||
                runtime.getSetting("AKASH_CLOSE_DEP") ||
                envConfig.AKASH_CLOSE_DEP,
            AKASH_CLOSE_DSEQ:
                process.env.AKASH_CLOSE_DSEQ ||
                runtime.getSetting("AKASH_CLOSE_DSEQ") ||
                envConfig.AKASH_CLOSE_DSEQ,
            AKASH_PROVIDER_INFO:
                process.env.AKASH_PROVIDER_INFO ||
                runtime.getSetting("AKASH_PROVIDER_INFO") ||
                envConfig.AKASH_PROVIDER_INFO,
            AKASH_DEP_STATUS:
                process.env.AKASH_DEP_STATUS ||
                runtime.getSetting("AKASH_DEP_STATUS") ||
                envConfig.AKASH_DEP_STATUS,
            AKASH_DEP_DSEQ:
                process.env.AKASH_DEP_DSEQ ||
                runtime.getSetting("AKASH_DEP_DSEQ") ||
                envConfig.AKASH_DEP_DSEQ,
            AKASH_GAS_OPERATION:
                process.env.AKASH_GAS_OPERATION ||
                runtime.getSetting("AKASH_GAS_OPERATION") ||
                envConfig.AKASH_GAS_OPERATION,
            AKASH_GAS_DSEQ:
                process.env.AKASH_GAS_DSEQ ||
                runtime.getSetting("AKASH_GAS_DSEQ") ||
                envConfig.AKASH_GAS_DSEQ,
            AKASH_MANIFEST_MODE:
                process.env.AKASH_MANIFEST_MODE ||
                runtime.getSetting("AKASH_MANIFEST_MODE") ||
                envConfig.AKASH_MANIFEST_MODE,
            AKASH_MANIFEST_PATH:
                process.env.AKASH_MANIFEST_PATH ||
                runtime.getSetting("AKASH_MANIFEST_PATH") ||
                envConfig.AKASH_MANIFEST_PATH,
            AKASH_MANIFEST_VALIDATION_LEVEL:
                process.env.AKASH_MANIFEST_VALIDATION_LEVEL ||
                runtime.getSetting("AKASH_MANIFEST_VALIDATION_LEVEL") ||
                envConfig.AKASH_MANIFEST_VALIDATION_LEVEL,
            AKASH_DEPOSIT:
                process.env.AKASH_DEPOSIT ||
                runtime.getSetting("AKASH_DEPOSIT") ||
                envConfig.AKASH_DEPOSIT,
        };
        ...
        throw new Error(
            `Failed to validate Akash configuration: ${errorMessage}`
        );
    }
}
