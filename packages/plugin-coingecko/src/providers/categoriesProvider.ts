...
async function fetchWithRetry(runtime: IAgentRuntime): Promise<CategoryItem[]> {
    ...
    for (let i = 0; i < MAX_RETRIES; i++) {
        try {
            ...
            lastError = error;
            elizaLogger.error(
                `Categories fetch attempt ${i + 1} failed:`,
                error
            );
    ...
    throw (
        lastError ||
        new Error("Failed to fetch categories after multiple attempts")
    );
}
...
async function getCategories(runtime: IAgentRuntime): Promise<CategoryItem[]> {
    try {
        ...
        const cached =
            await runtime.cacheManager.get<CategoryItem[]>(CACHE_KEY);
        if (cached) {
            return cached;
        }
        ...
        // Cache the result
        await runtime.cacheManager.set(CACHE_KEY, categories, {
            expires: CACHE_TTL,
        });
