...
describe("IrysService", () => {
    ...
    describe("Store String on Irys", () => {
        ...
        it("should retrieve data from Irys", async () => {
            const result = await irysService.getDataFromAnAgent(
                ["0x7131780570930a0ef05ef7a66489111fc31e9538"],
                []
            );
            console.log(
                "should retrieve data from Irys ERROR : ",
                result.error
            );
            expect(result.success).toBe(true);
        ...
        it("should get a response from the orchestrator", async () => {
            ...
            console.log(
                "should get a response from the orchestrator ERROR : ",
                result.error
            );
            expect(result.success).toBe(true);
            expect(result.data.length).toBeGreaterThan(0);
